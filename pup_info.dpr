...
begin
  ...
  if Record8 = Swap64($5343455546000001) then begin
    ...
    inMS.Read(Record8, SizeOf(Record8));
    WriteLn('Package version: ' + IntToHex(Record8, SizeOf(Record8)*2));
    inMS.Read(Record8, SizeOf(Record8));
    WriteLn('Image version:   ' + IntToHex(Record8, SizeOf(Record8)*2));
    ...
    WriteLn('Header length:   ' + IntToHex(HeaderLength8, SizeOf(HeaderLength8)*2));
    inMS.Read(Record8, SizeOf(Record8));
    WriteLn('Package length:  ' + IntToHex(Record8, SizeOf(Record8)*2));
    ...
    for i:=1 to FileCount8 do begin
      inMS.Read(Record8, SizeOf(Record8));
      Write(IntToHex(Record8, SizeOf(Record8)*2));
      FillChar(Hash, SizeOf(Hash), $00);
      inMS.Read(Hash, $20);
      Write(' ');
      for j:=0 to SizeOf(Hash)-1 do begin
        Write(IntToHex(Ord(Hash[j]), 2));
      end;
      WriteLn;
      inMS.Seek($18, soCurrent); // unknown
    end;
